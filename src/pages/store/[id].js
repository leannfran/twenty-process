...
const ProductDetail = ({ product }) => {
  ...
  const addToCartLocalStorage = async (products) => {
    ...
    if (quantityValue > 10000) {
      showDefaultAlert(
        "La cantidad de productos no puede ser mayor a 10000",
        "warning"
      );
      return;
    }
        ...
        <section className=" md:px-82  flex flex-col xl:flex-row max-w-[1300px]">
          ...
          <div className="  px-3 flex flex-col justify-between py-5 ">
            <div>
              ...
              <p className="pb-10 text-secondary">
                {" "}
                Los precios pueden variar según la cantidad. Para obtener una
                cotización exacta, contáctenos.
              </p>
            ...
            <div className=" h-32 xl:h-12 flex flex-col xl:flex-row justify-between gap-4 max-w-[676px]">
              <input
                type="number"
                value={quantityValue}
                className="md:w-60 h-full rounded-md p-2 border-2 border-gray-500 text-black"
                placeholder="Ingrese Cant. de productos"
                onChange={(e) => setQuantityValue(e.target.value)}
              />

              <button
                className="bg-primary hover:bg-secondary transition-colors md:w-60 h-full rounded-md text-center flex justify-center items-center text-white font-bold"
                onClick={() => addToCartLocalStorage(product)}
              >
